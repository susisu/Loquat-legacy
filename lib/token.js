...
function makeTokenParser (languageDef) {
    ...
    var fraction = lq.prim.label(
        lq.prim.then(
            lq.char.char("."),
            lq.prim.bind(
                lq.prim.label(lq.combinator.manyChar1(digit), "fraction"),
                function (digits) {
    ...
    var charControl = lq.prim.then(
        ...
        lq.prim.bind(
            lq.char.upper,
            function (code) {
                return lq.prim.pure(String.fromCharCode(code.charCodeAt(0) - "A".charCodeAt(0)));
            }
        )
    );
    ...
    var ident = lq.prim.label(
        lq.prim.bind(
            ...
            function (c) {
                return lq.prim.bind(
                    lq.prim.manyChar(languageDef.identLetter),
                    function (cs) {
    ...
    function caseString (name) {
        ...
        function walk (name) {
            if (name.length === 0) {
                ...
                return lq.prim.then(
                    lq.prim.label(caseChar(name[0]), lq.util.show(name)),
                    walk(name.substr(1))
                );
            }
        }
